...
@section('content')
    ...
    <div class="md:w-5/6">
        <div class=" py-6 m-2 md:py-2">
            <a href="{{route('admin.departments.create')}}" class="rounded border text-indigo-500 p-2 bg-transparent">Create
                Department</a>
        </div>
        @foreach ($departments as $department)
            <div class="md:flex border rounded shadow mb-1">
                <div class="m-2 p-2 py-2 md:w-5/6 ">
                    <a href="{{route('admin.departments.show', ['department'=>$department])}}"> {{ $department->getName()}}</a>
                </div>
